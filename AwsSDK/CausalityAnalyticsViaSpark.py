...
def DeleteCluster():
    ...
    if storage == 's3':
        ...
        bucket = s3_conn.get_bucket(reproduce_storage)
        k = Key(bucket)
        k.key = 'config.ini'
...
def createCluster():
    if storage == 's3':
        s3_conn = S3Connection(access_key,secret_key)
        try:
            bucket = s3_conn.create_bucket(reproduce_storage,location=REGION,policy='private')
        except:
            bucket = s3_conn.get_bucket(reproduce_storage)
...
def s3create():
    s3_conn = S3Connection(access_key,secret_key)
    bucket = s3_conn.create_bucket(reproduce_storage,location=REGION)
