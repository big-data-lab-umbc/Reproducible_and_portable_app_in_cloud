...
def lambda1_handler(event, context):
    ...
    send_command_to_master(masterInstanceId,\
        ...
        ssm_client)
    send_command_to_master(masterInstanceId,\
        s3_get_object(event['Configurations']['source_data']["bucketname"],event['Configurations']['source_data']["prefix"],"/home/ubuntu/"+event['Configurations']['source_data']["filename"],event['Configurations']['source_data']["version"]),\
        ssm_client)

    send_command_to_master(masterInstanceId,\
        "unzip /home/ubuntu/MultiGpus-Domain-Adaptation-main.zip -d /home/ubuntu/",\
        ssm_client)
    send_command_to_master(masterInstanceId,\
        "tar -xzvf /home/ubuntu/"+event['Configurations']['source_data']["filename"]+" -C /home/ubuntu/",\
        ssm_client)
    ...
    try:
        ...
        print('DA went wrong, please check logs.')
        raise e
    ...
    # caculate cost
    cost = (exe_time/3600*event['Configurations']['bill']['EC2_price']*event['Configurations']['instance_num'])\
