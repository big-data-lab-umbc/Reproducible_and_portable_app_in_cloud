...
ssh -i ./ssh/id_rsa -o "StrictHostKeyChecking no" azureuser@$ip "az storage blob upload --account-name $account_name --account-key $key -f /home/azureuser/$result_file_name -c reproducibility-log/$start_time -n $result_file_name"
...
record_json="{\"command_line\":\"command_line\", \"budgetary_cost\":$Budgetary_cost, \"execution_time\":$elapsed, \"performance_price_ratio\":$Performance_price_ratio, \"source_data\":\"https://causalityncerxyroxb2sg.blob.core.windows.net/reproducibility-log/$start_time/command.sh\", \"source_data_verion\":null, \"program_result\":\"https://causalityncerxyroxb2sg.blob.core.windows.net/reproducibility-log/$start_time/$result_file_name\", \"program_result_version\":null, \"reproducibility_config\":\"https://causalityncerxyroxb2sg.blob.core.windows.net/reproducibility-log/$start_time/parameter.json\", \"reproducibility_config_version\":null}"
